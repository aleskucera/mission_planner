...
from pathsolver.src.utils import get_point_line
...
class Way:
    ...
    def is_road(self):
        ...
        """
        if (
            self.tags.get("highway", None)
            and not self.tags.get("highway", None) in FOOTWAY_VALUES
        ):
            return True
    ...
    def is_footway(self):
        ...
        if (
            self.tags.get("highway", None)
            and self.tags.get("highway", None) in FOOTWAY_VALUES
        ):
            return True
    ...
    def is_barrier(self, yes_tags, not_tags, anti_tags):
        ...
        if any(
            ...
            and (
                ...
                or (
                    "*" in yes_tags[key] and not self.tags[key] in not_tags.get(key, [])
                )
            )
